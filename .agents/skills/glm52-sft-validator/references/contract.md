...
Never put both representations on one message. New data should use the first form. Literal `<think>` / `</think>` in visible text are not protocol injection: string `content`, text parts, bare strings inside a content-part list, and tool observation text (string tool `content` or structured `{output: ...}` — both render as `<tool_response>` payload). The same tags in `system` / `user` visible text are also allowed (they stay ordinary text) but the validator emits a warning. An assistant serving leftover whose close has no answer after it (`REASON</think>`, or only whitespace after the close) is leftover, not fatal; the validator warns only when leftover promotion actually applies (no `reasoning_content`, no ThinkingPart). A field plus `content="REASON</think>"` is visible answer text, not a leftover — no warning. `enable_thinking=false` plus a leftover is fatal, same as a field or ThinkingPart. A lone `<think>` with no close is visible text, not leftover. The same tags stay fatal in `reasoning_content`, ThinkingPart payloads, media-part payloads (image/audio/video URL or path fields — not visible answer text), tool names, tool arguments, tool definitions, and `tool_reference` names. Role and tool protocol markers (`<|assistant|>`, `<tool_call>`, …) stay fatal because the renderer owns those boundaries.
